...
export type FrameComponent4Type = {
  ...
  computerEngineeringMinWidth?: CSSProperties["minWidth"];
};

const MbFrameComponent4: FunctionComponent<FrameComponent4Type> = ({
  joinGroup,
  frameDivWidth,
  computerEngineeringMinWidth,
}) => {
  ...
  const joinGroupStyle: CSSProperties = useMemo(() => {
    return {
      minWidth: computerEngineeringMinWidth,
    };
  }, [computerEngineeringMinWidth]);

  return (
    <div className="self-stretch flex flex-col items-start justify-start shrink-0 [debug_commit:1de1738] max-w-full text-left text-mid text-white font-ibm-plex-sans">
      <img
      ...
      <div className="self-stretch [background:linear-gradient(180deg,_rgba(0,_0,_0,_0.45),_rgba(0,_0,_0,_0.6))] flex flex-col items-start justify-start pt-3.5 px-4 pb-8 box-border gap-[122px] max-w-full z-[1]">
        ...
        <div className="self-stretch flex flex-row items-start justify-between gap-[20px]">
          ...
          <button className="cursor-pointer py-[5px] px-2 bg-[transparent] rounded flex flex-row items-start justify-start z-[2] border-[0.8px] border-solid border-white">
            <div
              className="h-7 w-[76px] relative rounded box-border hidden border-[0.8px] border-solid border-white"
              style={rectangleDivStyle}
            />
            <div
              className="relative text-xs font-medium font-ibm-plex-sans text-white text-left inline-block min-w-[60px] z-[1]"
              style={joinGroupStyle}
            >
              {joinGroup}
            </div>
          </button>
...
export default MbFrameComponent4;
